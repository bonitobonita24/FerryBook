...
7. Ticketing Staff can create a walk-in booking at the terminal (or via Counter PWA `counter.fandsmarine.ph` on mobile): the screen opens directly on a **today's-sailings strip** for the staff member's logged-in port (e.g. BAT-NAS for Marisol Hidalgo at Nasugbu) — each sailing row carries its manifest state: **(a) Departed · manifest declared ✓** (info-only, greyed), **(b) Active sailing · "Boarding now"** (auto-selected, the *only* row staff can book against), **(c) Next sailing** (visible but locked-with-tooltip until the active sailing's manifest is declared/finalized by the Boarding Officer). The active sailing is computed as the first row in the day where `!departed && !manifestDeclared`; staff cannot manually re-select a different sailing — this prevents accidental booking onto a later sailing while the active one still has seats and stops staff from circumventing the manifest cutoff → pick class → set passenger count → fill per-passenger details (name / age / sex / ID type / ID number / passenger type) → **assign each passenger to a specific seat via `assignSeat(passengerIndex, seatId)` on the live seat map for the chosen class (Open Air 10×8 / Aircon 10×5 / VIP 3×4); the seat map shows already-taken seats greyed and unselectable, and each passenger row in the form carries its assigned seat ID inline once picked; staff can re-tap to reassign** → pick payment method (Cash / Card / GCash) → if cash, enter cash tendered and the screen computes change → issue printed e-ticket with QR + booking ref + per-passenger ticket numbers → seats deducted from inventory. Nasugbu staff cannot create Calatagan bookings and vice versa. Error: active sailing's manifest gets declared mid-flow → the next sailing auto-promotes to active and the in-progress booking is reset (with a confirm dialog). Error: every same-day sailing has departed → walk-in booking blocked with message "Voyage closed for boarding."
...
- **Print preview (A4 landscape continuous-form)** — "Print preview" button on Confirmation screen and Booking Detail screen toggles a print-optimized rendering: A4 landscape, continuous-form layout suitable for terminal dot-matrix or thermal printers, all passenger rows on one sheet (booking ref + QR top-left, vessel + port + datetime header band, passenger table with seat IDs and IDs-to-bring column, fare breakdown footer, watermark "F&S MARINE — OFFICIAL E-TICKET"). Used by Counter staff to hand a paper copy to walk-in customers and by online customers who prefer printing
...
> **✅ Confirmed in client meeting 2026-05-24:** Three recovery options — **(A) 100% Refund via Xendit, (B) Free same-route Reschedule with fare difference waived, (C) Travel Credit with 12-month validity** — and the **72-hour auto-credit default** for non-responsive customers are accepted as V1 behavior.
...
- Three modes:
  ...
  - **C. Final manifest** — MARINA MC-180 compliant document with vessel name, registry, master's name + license, voyage number, CPC, weather, distance, ETA, total authorized capacity per MARINA MC-180 §7, passenger list with per-person status (Boarded / NoShow / Walkup), and **two pre-printed wet-ink signature lines** for **Boarding Officer** and **Master/Captain** (per updated PCG/MARINA practice — see Manifest print-preview note below). Exported as PDF for PCG/MARINA submission
- **Manifest print-preview** — the Boarding Officer screen surfaces a "Print manifest" button (also wired up via an `onShowManifest` prop from the parent StaffBoardingScreen so the same preview is reachable from the Check-In flow's "show manifest" affordance) that opens a print-optimized A4 portrait rendering: header band (vessel registry block + voyage ID + Batangas port + departure time + CPC + weather + ETA), full passenger table (Name · Type · ID Type/No · Seat · Boarding Status), and a footer with two empty signature lines labeled "Boarding Officer · printed name · date/time" and "Master / Captain · printed name · date/time". Officer prints, both signatories sign in wet ink at the dock, the signed page is scanned/photographed and uploaded back to the Voyage record to attest finalization
- **Signature capture model — wet-ink only in V1 (changed 2026-05-24):** the previous on-screen `SignaturePad` component has been **removed entirely** from the customer/staff UI. Both required signatures (Boarding Officer + Master/Captain) are collected on the printed manifest in wet ink per current PCG/MARINA practice; the app stores the scanned/uploaded signed-PDF and the metadata (who finalized, when, from which device) but no longer captures vector signature paths in the browser
...
- **📋 Placeholder pending client input (meeting 2026-05-24):** Client to provide actual personnel assignments per port for the three port-scoped roles. Seed data structure below — names and contact info TBD:
  - **Nasugbu Port (BAT-NAS):** Walk-In / Ticketing Staff (TBD), Check-In Staff (TBD), Boarding Officer (TBD)
  - **Calatagan Port (BAT-CAL):** Walk-In / Ticketing Staff (TBD), Check-In Staff (TBD), Boarding Officer (TBD)
  - Each port needs at minimum 1 Walk-In staff, 1 Check-In staff (may be same person), and 1 Boarding Officer for operational coverage. Backups optional but recommended. Master/Captain assignments per vessel also pending (used on MC-180 signature pad).
...
- **Cancellation Policy tab** — admin-configurable values, all editable in-place via numeric inputs with live preview against current bookings:
  > **✅ Confirmed in client meeting 2026-05-24:** The default pre-departure refund ladder (50% cap ≥120h, 40% 96-120h, 30% 72-96h, 20% 48-72h, 10% 24-48h, 0% <24h) and the **50% flat reschedule fee** are accepted as V1 defaults. All tier percentages and the reschedule-fee percentage remain admin-configurable so the client can adjust later without code changes.
...
ManifestFinalization: id, voyageId, role (BoardingOfficer / Master), userId (FK to User — the Boarding Officer who attests the wet-ink signing event; for Master role, FK references the User row representing the captain), attestedAt (when the staff confirmed in-app that the printed manifest was signed at the dock), signedManifestPdfUrl (nullable; URL to the scanned/photographed wet-ink-signed PDF uploaded back to the system as evidence — required to finalize a voyage; stored encrypted at rest), ipAddress, deviceInfo. **Renamed from ManifestSignature 2026-05-24 — the prior in-browser `signatureSvg` field is removed; both required attestations (BoardingOfficer + Master) are now collected on the printed manifest in wet ink per PCG/MARINA practice (see Boarding Officer module). Both attestations required to finalize a voyage's MARINA MC-180 manifest. The signed PDF is the legal record.**
...
| 9 | Booking review | Mobile First | Customer-facing pre-payment screen with explicit Batangas departure port + assigned seat numbers shown |
...
| 13 | Booking confirmation / E-ticket | Mobile First | Shown at boarding — must work on phone — prominently displays assigned Batangas port |
...
| 34 | Refund queue | Mobile Ready | Data table with filters (pre-departure RR + no-show NSR + reschedule NSB) |
...
/meetings/[slug]           shareable client meeting minutes (added 2026-05-24, mockup-only deliverable, client-side routing via lightweight wrapper component, native Web Share API on the share button with clipboard fallback; lives outside the core booking-app routes, intentionally NOT under /admin or /account, and does not interact with any FBM business data — used as a sharable handoff artifact between Powerbyte and the client). The wrapper renders a dark sticky header (`#1A1A2E` bg + coral Share pill) and a 820px-max centered body. **Two parallel implementations ship in `docs/vercel_mockup/`:** (1) the inline route at `docs/vercel_mockup/src/MeetingMinutesApp.jsx` + `docs/vercel_mockup/src/screens/MeetingMinutes20260524.jsx` mounted by `main.jsx`'s tiny path-match router on `/meetings/<slug>`, and (2) a **standalone Vite app** at `docs/vercel_mockup/meetingminutes/20260524/` (its own `App.jsx`, `index.html`, `vite.config.js`, `tailwind.config.js` and `package.json`) for deploying a single-meeting snapshot independently of the main mockup. Both render the same minutes content with the same status taxonomy
...
Complex UI components:     Kibo UI (rich text editor for admin terms/promo editing; file dropzone for ID photo upload AND signed-manifest PDF upload — MC-180 attestations are wet-ink on printed manifest in V1, no in-browser signature pad). **i18n scaffolding planned with next-intl (English-only strings in V1, Tagalog translations as V1.1 fast-follow — pending client confirmation, see Out of Scope #9)**
...
## Meeting Minutes Deliverable (mockup-only, separate from main app)

> Added 2026-05-24. NOT a production booking-app feature — a sharable artifact for Powerbyte → client handoff.

- **Route(s):** `/meetings/<slug>` in the mockup app (e.g. `/meetings/2026-05-24`); also deployable standalone from `docs/vercel_mockup/meetingminutes/<date>/`
- **Status taxonomy** (used to tag every discussion item in the minutes — colors mirror the in-app semantic palette so the client reads the same color language as during the demo):
  - 🟡 **Needs Client Decision** — `#D97706` on `#FEF3C7`
  - 🔵 **For Review** — `#1E40AF` on `#DBEAFE`
  - 🟢 **Confirmed** — `#16A34A` on `#DCFCE7`
  - 🟣 **New Feature Request** — `#7C3AED` on `#EDE9FE`
  - 🟠 **Scope Change** — `#FF385C` (coral) on `#FFE5E9`
- **Content per minutes page:** meeting metadata (date, attendees, location), one section per discussion item with a status badge, a body, and (when applicable) an Action Items block listing the client's to-do tasks
- **Share affordance:** native `navigator.share` when available, falls back to `navigator.clipboard.writeText` with a "✓ Link copied" toast
- **Hosting note:** the minutes deliverable ships under the same Vercel project as the mockup; the standalone meetingminutes app is a separate buildable target if a single-meeting URL needs to be deployed in isolation
- **Not a product feature:** this section exists so future spec readers don't confuse the meeting-minutes route with a customer- or admin-facing booking-app feature. Do NOT add it to the role permission matrix, audit log event types, or any business-data flow. It is a static-content artifact

## Out of Scope
The following are explicitly NOT part of V1 and must not be implemented:

1. Cargo and vehicle (RORO) booking — passengers only
   > **⚠️ Pending Scope Change (client meeting 2026-05-24):** Client wants passengers to indicate "With Vehicle" + select vehicle type during online booking for *forecasting only* (no online payment for vehicle). Actual vehicle fee is assessed by Check-In Staff at the port based on physical inspection. Vehicle ticket is linked as a sub-ticket under the passenger's main ticket. Only Walk-In / Check-In Staff can issue vehicle billing. **Status: Pending Client Confirmation — this item remains Out of Scope until confirmation.** See pending notes in Customer Booking Flow and Ticketing Staff modules.
...
9. Multi-currency or multi-language — PHP only, English only
   > **⚠️ Pending Scope Change (client meeting 2026-05-24):** Client requested a Filipino (Tagalog) language toggle for the customer-facing app. Dev recommendation: scaffold V1 i18n-ready using **next-intl** (English-only string keys, no translations shipped); Tagalog translations land as a V1.1 fast follow-up immediately after launch. Admin/back-office surfaces remain English-only. Currency stays PHP-only. **Status: Pending Client Confirmation — this item remains Out of Scope until confirmation.**
10. Third-party reseller API integration (Barkota, 12Go, PHBus)
11. Live GPS vessel tracking visible to customers
12. Seat-map or specific seat selection — capacity-by-class only (seats assigned by terminal staff at boarding)
